      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <ScanLine size={16} className="text-[#F84464]" />
            <span className="text-[11px] font-bold uppercase tracking-wider text-gray-400">
              Access Control & Gate Turnstile
            </span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-black text-gray-900 tracking-tight">
            Cinema Gate Ticket Scanner
          </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="bg-gray-50 text-gray-800 py-3.5 px-6 border-b border-gray-200 flex items-center justify-between">
              <div className="text-sm font-bold text-gray-900 flex items-center gap-2.5">
                <Camera size={18} className="text-[#F84464]" />
                <span>Optical Gate Scanner Viewfinder</span>
              </div>
              <span className="inline-flex items-center gap-1.5 text-[10px] bg-emerald-50 text-emerald-700 font-mono px-2.5 py-1 rounded-full border border-emerald-200">
            ...
            <div className="p-6">
              ...
              <div className="relative w-full aspect-[16/9] bg-[#0A0C13] rounded-2xl overflow-hidden flex flex-col items-center justify-center border border-gray-300 shadow-[inset_0_0_35px_rgba(0,0,0,0.9)]">
                {/* Targeting Corners */}
                ...
                <div className="text-center z-10 p-4">
                  <div className="w-14 h-14 rounded-2xl bg-white/[0.08] border border-white/20 flex items-center justify-center mx-auto mb-3 shadow-lg">
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <input
                      ...
                      className="w-full text-sm font-mono tracking-wider font-bold py-2.5 px-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#F84464]/20 focus:border-[#F84464] bg-gray-50 text-gray-900 placeholder-gray-400 uppercase placeholder:normal-case shadow-xs"
                    />
                  </div>
                  ...
                  <Button
                    ...
                    variant="primary"
                    isLoading={loading}
                    className="px-6 font-bold text-xs shrink-0 rounded-lg bg-[#F84464] hover:bg-[#E03A58] text-white shadow-sm"
                  >
                    Validate Ticket
                  </Button>
                </div>
              ...
              <div className="mt-6 pt-4 border-t border-gray-200">
                <div className="flex items-center justify-between mb-2.5">
                  ...
                  <span className="text-[11px] text-gray-500 font-medium">Test validation</span>
                </div>

                {testTicketsLoading ? (
                  <p className="text-xs text-gray-500">Loading active reservations...</p>
                ) : testTickets.length === 0 ? (
                  <p className="text-xs text-gray-500">No active bookings found to test.</p>
                ) : (
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {testTickets.map((t) => (
                      <button
                        ...
                        className={`p-2.5 rounded-lg border text-left text-xs transition flex flex-col justify-between group shadow-xs cursor-pointer ${
                          t.ticketValidated
                            ? 'bg-emerald-50 border-emerald-200 text-emerald-800 hover:bg-emerald-100'
                            : 'bg-gray-50 border-gray-200 hover:border-[#F84464] hover:bg-white text-gray-900'
                        }`}
                      >
                        <div className="flex items-center justify-between w-full">
                          <span className="font-mono font-bold text-[11px] text-gray-900 group-hover:text-[#F84464] transition-colors">
                            {t.bookingId}
                          </span>
                          {t.ticketValidated ? (
                            <span className="w-2 h-2 rounded-full bg-emerald-500 shrink-0" title="Already Checked In" />
                          ) : (
                            <span className="w-2 h-2 rounded-full bg-amber-500 shrink-0" title="Pending Check-in" />
                          )}
                        </div>
                        <p className="text-[10px] text-gray-500 mt-1.5 line-clamp-1 font-medium">
                          {t.movieTitle}
                        </p>
                      </button>
                    ))}
        ...
        <div className="lg:col-span-5 space-y-6">
          {validationResult ? (
            <motion.div
              ...
              className={`rounded-xl border p-5 shadow-sm transition-all duration-300 relative overflow-hidden bg-white ${
            ...
            >
              ...
              <div className="flex items-center gap-3.5 pb-4 border-b border-gray-100">
                ...
                <div>
                  <h3 className="font-black text-lg uppercase tracking-wider leading-tight text-gray-900">
                    {validationResult.status === 'SUCCESS'
                      ? 'ACCESS GRANTED'
                      : validationResult.status === 'DUPLICATE'
                      ? 'DUPLICATE SCAN'
              ...
              {validationResult.booking ? (
                <div className="mt-4 space-y-3 text-xs">
                  <div className="bg-gray-50 rounded-xl p-4 border border-gray-200 space-y-2.5 text-gray-800">
                    <div className="flex items-center justify-between pb-2 border-b border-gray-200">
                      ...
                      <span className="bg-white text-gray-700 border border-gray-200 text-[10px] font-bold px-2 py-0.5 rounded-full uppercase">
                    ...
                    <div className="flex items-center gap-2 text-[11px] text-gray-600 font-mono">
                      ...
                      <span>•</span>
                      <span className="font-bold text-gray-900">{validationResult.booking.showtime}</span>
                    </div>

                    {/* Assigned Seats Matrix */}
                    <div className="pt-2 border-t border-gray-200 flex items-center justify-between">
                      <span className="text-gray-500 font-medium text-[11px]">Assigned Seats:</span>
                      <div className="flex flex-wrap gap-1">
                        {(validationResult.booking.seats || []).map((s) => (
                          <span
                            key={s}
                            className="bg-white border border-gray-300 text-gray-900 font-mono font-bold text-xs px-2 py-0.5 rounded-md shadow-xs"
                          >
                            {s}
                          </span>
                        ))}
                  ...
                  <div className="mt-4 pt-3 border-t border-gray-200 flex justify-end">
                    <Button
                      ...
                      className="text-xs font-semibold text-gray-700 hover:text-gray-900 border-gray-300 hover:bg-gray-50"
                    >
            ...
            <div className="rounded-xl border border-dashed border-gray-300 p-8 text-center bg-white shadow-sm">
              <div className="w-12 h-12 rounded-xl bg-rose-50 text-[#F84464] border border-rose-200 flex items-center justify-center mx-auto mb-3">
                <Ticket size={24} />
              </div>
              <h3 className="font-bold text-gray-900 text-sm">Gate Validation Standby</h3>
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              ...
              <div className="divide-y divide-gray-100 max-h-56 overflow-y-auto">
                {recentScans.map((scan, idx) => (
                  <div key={idx} className="p-3 flex items-center justify-between text-xs hover:bg-gray-50 transition-colors">
                    <div>

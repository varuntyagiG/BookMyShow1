            ...
            <div ref={searchBoxRef} className="relative flex-1 hidden sm:block max-w-xl">
              <form onSubmit={handleSearchSubmit} className="relative">
                ...
                >
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-20 py-2.5 bg-transparent text-white text-[13px] font-normal placeholder-slate-400 focus:outline-none transition-all"
                  />
                  
                  {/* Right side helper (clear, Ask AI button, and Ctrl+K shortcut) */}
                  <div className="absolute right-2.5 flex items-center gap-1.5">
                    {searchInput ? (
                      ...
                      <>
                        <button
                          type="button"
                          onClick={() => window.dispatchEvent(new CustomEvent('open-cinematch'))}
                          className="px-2.5 py-1 rounded-lg bg-gradient-to-r from-violet-600 to-[#F84464] hover:from-violet-500 hover:to-rose-500 text-white text-xs font-semibold tracking-normal flex items-center gap-1.5 cursor-pointer transition-all hover:scale-105 active:scale-95 shadow-sm"
                          title="Ask CineMatch AI for Recommendations"
                        >
              ...
              {searchFocused && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-[#1B1D28]/98 backdrop-blur-2xl text-white rounded-2xl shadow-[0_25px_60px_-15px_rgba(0,0,0,0.8)] border border-white/10 py-3 z-50 animate-in fade-in zoom-in-95 duration-150">
                  {/* CineMatch AI Banner in Search Dropdown */}
                  <div className="mx-3 mb-2 p-2.5 rounded-xl bg-gradient-to-r from-violet-950/60 via-slate-900 to-rose-950/50 border border-violet-500/30 flex items-center justify-between gap-2 shadow-inner">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-lg bg-gradient-to-tr from-violet-600 to-[#F84464] flex items-center justify-center text-white shrink-0 shadow-xs">
                        <Sparkles className="w-3 h-3 text-amber-300" />
                      </div>
                      <span className="text-[11px] text-slate-200 font-medium">Can't decide? Let CineMatch AI recommend by vibe</span>
                    </div>
                    <button
                      type="button"
                      onClick={() => {
                        setSearchFocused(false);
                        window.dispatchEvent(new CustomEvent('open-cinematch'));
                      }}
                      className="px-2.5 py-1 bg-gradient-to-r from-violet-600 to-[#F84464] hover:from-violet-500 hover:to-rose-500 text-white text-[10px] font-black rounded-lg shadow-sm hover:scale-105 active:scale-95 transition-all cursor-pointer shrink-0"
                    >
                      Ask AI ✨
                    </button>
                  </div>

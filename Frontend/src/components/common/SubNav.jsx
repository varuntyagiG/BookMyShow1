...
import { Sparkles, Building2, Tag, Gift, Tv, Film } from 'lucide-react';
...
export default function SubNav() {
  ...
  return (
    <nav className="bg-[#0A0C13]/92 backdrop-blur-2xl text-gray-300 text-xs border-b border-white/[0.06] sticky top-16 z-30 shadow-[0_6px_25px_-5px_rgba(0,0,0,0.5)] select-none">
          ...
          <div className="flex items-center space-x-7 overflow-x-auto no-scrollbar py-1">
            <Link
              ...
              className={`hover:text-white transition-all cursor-pointer relative py-2 font-semibold shrink-0 flex items-center ${
                currentPath === '/' ? 'text-white font-black' : 'text-gray-400 hover:text-gray-200'
              }`}
            >
              <span>All</span>
              {currentPath === '/' && (
            ...
            {categories.map((cat) => {
              ...
              return (
                ...
                >
                  ...
                  {cat.badge && (
                    <span 
                      className={`px-1.5 py-0.5 text-[8.5px] rounded-md font-black uppercase tracking-wider shadow-xs ${
                        cat.badge === 'NEW' 
            ...
            {utilityLinks.map((item) => {
              ...
              return item.path ? (
                <Link
                  ...
                  to={item.path}
                  onClick={() => playPop()}
                  className={`transition-all flex items-center gap-1.5 font-bold cursor-pointer text-xs ${
                    item.highlight 
                      ? 'bg-white/[0.07] hover:bg-white/[0.14] border border-white/10 px-3 py-1 rounded-full text-gray-200 hover:text-white shadow-xs hover:border-[#F84464]/50' 
